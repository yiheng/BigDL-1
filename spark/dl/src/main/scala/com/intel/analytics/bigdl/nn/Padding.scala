...
object Padding {
  def apply[@specialized(Float, Double) T: ClassTag](
    ...
    if (pad > 0) new Padding[T](Array(dim), Array(0, pad), nInputDim, value, nIndex)
    else new Padding[T](Array(dim), Array(pad, 0), nInputDim, value, nIndex)
  }
}

object PaddingMulDim {
  def apply[@specialized(Float, Double) T: ClassTag](
    dim: Array[Int],
    pad: Array[Int],
    nInputDim: Int,
