...
class Graph[T: ClassTag](inputs : Seq[ModuleNode[T]],
  ...
  override def updateOutput(input: Activity): Activity = {
    ...
    if (inputs.length == 1) {
      ...
      inputs(i).value.updateOutput(inputsBP(i))
      i += 1
    } else {
    ...
    while(i < executions.length) {
      ...
      inputsBP(i) = if (node.prevNodes.length == 1) {
        node.prevNodes.head.value.output.toTensor[T]
      } else {
        seqToTable(node.prevNodes.map(_.value.output))
      }
      node.value.updateOutput(inputsBP(i))
      i += 1
    }
    ...
    output = if (outputs.length == 1) {
      ...
      seqToTable(outputs.map(_.value.output))
    }
    output
  }
  ...
  override def updateGradInput(input: Activity, gradOutput: Activity): Activity = {
    ...
    if (outputs.length == 1) {
      ...
      outputs(j).value.updateGradInput(inputsBP(i), gradOutput.toTensor)
      j -= 1
      i -= 1
    } else {
      ...
      while(j >= 0) {
        outputs(j).value.updateGradInput(inputsBP(i), gradOutputTable[Tensor[T]](j))
        j -= 1
        i -= 1
      }
    }
    ...
    while(i >= 0) {
      ...
      curNode.nextNodes.foreach(n => {
        val nextGradOutput = if (n.prevNodes.length == 1) {
          n.prevNodes(0).value.gradInput.toTensor
        } else {
          val nextGradOutputTable = n.prevNodes(0).value.gradInput.toTable
      ...
      curNode.value.updateGradInput(inputsBP(i), curGradOutput)
      i -= 1
    }

    if (inputs.length == 1) {
      inputs(0).value.gradInput
    } else {
      seqToTable(inputs.map(_.value.gradInput))
    }
  }
  ...
  override def accGradParameters(input: Activity, gradOutput: Activity, scale: Double): Unit = {
    ...
    if (outputs.length == 1) {
      // Expect a tensor here
      outputs(j).value.accGradParameters(inputsBP(i), gradOutput.toTensor)
      j -= 1
      i -= 1
    } else {
      val gradOutputTable = gradOutput.toTable
      require(gradOutputTable.length() == outputs.length,
        "gradient tensor number is not equal to outputs node number")
      while(j >= 0) {
        outputs(j).value.accGradParameters(inputsBP(i), gradOutputTable[Tensor[T]](j))
        j -= 1
        i -= 1
      }
    }
    ...
    while(i >= 0) {
      ...
      curNode.value.accGradParameters(inputsBP(i), curGradOutput)
      i -= 1
    }
  }
  ...
  private val executions = backGraph.topologySort.filter(_.value != null).reverse
  modules.appendAll(executions.map(_.value.asInstanceOf[AbstractModule[Activity, Activity, T]]))
