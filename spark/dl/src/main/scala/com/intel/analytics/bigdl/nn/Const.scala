 ...
 * @param value the constant tensor to be returned
 */
...
class Const[T: ClassTag](value: Tensor[T])(implicit ev: TensorNumeric[T])
  ...
  override def updateGradInput(input: Activity, gradOutput: Tensor[T]): Table = null
}

object Const {
  def apply[@specialized(Float, Double) T: ClassTag](value: Tensor[T])
